...
void RBT<T>::singleCR(RBTNode<T> *&point) {
    ...
    //Grandpa(1) becomes parents(1) right child and is newParent
    // Parent(1) becomes Grandpa(2) newGrandpa
    // Parents right child becomes newParent left child
    parent->parent = grandparent->parent;
    grandparent->parent = parent;
    grandparent->right - parent->left;
    parent->left = grandparent;
    // We can just assign the left parent of the G'pa to G'pa
    if(grandparent->left != nullptr){
        grandparent->left->parent = grandparent;
    }
    // There is only nodes, so we are at the root, just change the root as the parent
    if(parent->parent == nullptr){ 
        root = parent;
    }
    // Similar to first statement, but we are checking if the left node is the grandparent to assign a new parent
...
void RBT<T>::singleCCR(RBTNode<T> *&point) {
    ...
    // TODO: ADD ROTATION CODE HERE
    // Same as Clockwise Rotation... but Counter.
    parent->parent = grandparent->parent;
    grandparent->parent = parent;
    grandparent->right - parent->left;
    parent->left = grandparent;
...
void RBT<T>::insert(const T &toInsert, RBTNode<T> *&point, RBTNode<T> *parent) {
    if (point == nullptr) {               // leaf location is found so insert node
        ...
        // I'm pretty sure this violates a rule... I think 3
        if(curr_node->parent->left->color == RED){
            parent->color = BLACK;
        ...
        // This one is def 4.
        else if(curr_node->parent->right->color == BLACK){
            parent->color = RED;
